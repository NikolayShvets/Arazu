...
void Setup()
{
	...
	character.ch_x = 10; // стоим в точке с координатами (2,2). ch_x - номер строки, ch_y - номер элемента в строке, порядковый номер которой ch_x
	character.ch_y = 10;
...
void DrawMap(char skin, string map_name, move_var ch_move)
{
	...
	for (int i = 0; i < height; ++i)
	{
		...
		//если номер текущей строки больше или равен чем координата х минус область видимости по х И  номер текущей строки меньше или равен чем координата х плюс область видимости по х
		if ((i >= character.ch_x - character.visibility_x) && (i <= character.ch_x + character.visibility_x) && (i != 0) && (i != height - 1))
		{
			//для каждого элемента этой строки
			for (int n = 0; n < current_draw_map[i].size(); ++n)
			{
				// номер текущего элемента строки меньше или равен чем у - О.В. по у ИЛИ номер текущего элемента больше или равен чем у + О.В.
			...
			for (int n = 0; n < current_draw_map[i].size(); ++n)
			{
				if ((n != character.ch_y) && (n != 0) && (n != width - 1) && (i != 0) && (i != height - 1))
				{
		...
		//иначе, приусловии, что текущая строка не является границей карты
		...
		//иначе отрисовываем строку как есть (границы)
		else cout << current_draw_map[i] << endl;
...
void Input()
{
	//если на клавиатуре была нажата клавиша
	if (_kbhit())
	{//в зависимости от введенного символа выполняем следующее (меняем поле ch_move структуры Character)
		switch (_getch())
		{
		case 'a': character.ch_move = LEFT;/* map_change = true;*/  break;
		case 'w': character.ch_move = UP;/* map_change = true;*/ break;
		case 'd': character.ch_move = RIGHT;/* map_change = true;*/ break;
		case 's': character.ch_move = DOWN; /*map_change = true; */break;
